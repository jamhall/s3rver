...
describe('S3rver Tests', function () {
  ...
  it('should upload a managed upload <=5MB', function (done) {
    const params = { Bucket: buckets[0], Key: 'multi/directory/path/multipart', Body: Buffer.alloc(5e+6) }; // 5MB
  ...
  it('should upload a managed upload >5MB (multipart upload)', function (done) {
    const params = { Bucket: buckets[0], Key: 'multi/directory/path/multipart', Body: Buffer.alloc(2e+7) }; // 20MB
    s3Client.upload(params, function (err, data) {
      (/"[a-fA-F0-9]{32}"/).test(data.ETag).should.equal(true);
      if (err) {
        return done(err);
