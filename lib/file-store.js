    ...
    async.eachSeries(filteredKeys, function (key, callback) {
        fs.readFile(path.join(key, METADATA_FILE), function (err, data) {
          if (options.delimiter) {
            ...
            var skip = (options.prefix? match.substr(options.prefix.length): match).indexOf(options.delimiter) > -1;
            if (skip) {
        ...
        if (options.marker) {
          ...
          return done(null, (found) ? matches.slice(startAt, options.maxKeys) : []);
        }
        done(null, matches.slice(0, options.maxKeys));
      }
    );
  };
