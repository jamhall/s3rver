...
const FileStore = function (rootDirectory) {
  const CONTENT_FILE = '.dummys3_content', METADATA_FILE = '.dummys3_metadata', Bucket = require('./models/bucket'), S3Object = require('./models/s3-object');
  ...
  const getObject = function (bucket, key, range, done) {
    ...
    fs.stat(filePath, function (err, stats) {
      ...
      async.parallel([
        function (callback) {
          const readStream = fs.createReadStream(path.join(filePath, CONTENT_FILE), options);
    ...
    if (options.delimiter && options.prefix) {
      ...
      _.forEach(keys, function (key) {
        ...
        if (truncatedKey.indexOf(options.delimiter) > -1) {
          const commonPrefix = truncatedKey.substring(0, truncatedKey.indexOf(options.delimiter) + 1);
          if (commonPrefixes.indexOf(commonPrefix) == -1) {
            commonPrefixes.push(commonPrefix);
            commonPrefixes.sort();
          }
  ...
  const createMetaData = function (data, done) {
    const contentFile = data.contentFile, type = data.type, encoding = data.encoding, disposition = data.disposition, metaFile = data.metaFile, headers = data.headers;
    async.parallel([
      function (callback) {
  ...
  const combineObjectParts = function (bucket, key, uploadId, parts, req, done) {
    const sortedParts = _.sortBy(parts, function (part) { return part.number; });
    const partPaths = _.map(sortedParts, function (part) {
      return path.resolve(getBucketPath(bucket.name), uploadId + '_' + part.number);
    });
    const partStreams = _.map(partPaths, function (partPath) { return fs.createReadStream(path.join(partPath, CONTENT_FILE)); });
    ...
    const contentFile = path.join(dirName, CONTENT_FILE);
    const metaFile = path.join(dirName, METADATA_FILE);
    const writeStream = combinedPartsStream.pipe(fs.createWriteStream(contentFile));
    writeStream.on('error', function (err) {
      return done(err);
    });
    writeStream.on('close', function () {
