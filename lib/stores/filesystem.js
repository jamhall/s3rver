...
class FilesystemStore {
  ...
  async retrieveMetadata(bucket, key) {
    ...
    const metadataPath = path.join(
      this.getBucketPath(bucket),
      key,
      format(S3RVER_SUFFIX, "metadata.json")
    );
  ...
  async storeMetadata(bucket, key, metadata, md5) {
    ...
    const md5Path = path.join(
      this.getBucketPath(bucket),
      key,
      format(S3RVER_SUFFIX, "object.md5")
    );
  ...
  async copyObject(
    ...
    if (srcKeyPath !== destKeyPath) {
      ...
      await fs.copy(
        path.join(srcKeyPath, format(S3RVER_SUFFIX, "object")),
        path.join(destKeyPath, format(S3RVER_SUFFIX, "object"))
      );
    }
    ...
    if (replacementMetadata) {
      ...
      if (srcKeyPath !== destKeyPath) {
        await fs.copy(
          path.join(srcKeyPath, format(S3RVER_SUFFIX, "metadata.json")),
          path.join(destKeyPath, format(S3RVER_SUFFIX, "metadata.json"))
        );
      }
      return this.retrieveMetadata(destBucket, destKey);
    }
  ...
  async deleteObject(bucket, key) {
    ...
    await Promise.all(
      [
        ...
        path.join(keyPath, format(S3RVER_SUFFIX, "metadata.json"))
      ].map(filePath =>
  ...
  async retrieveSubresource(bucket, key, resourceType) {
    const resourcePath = path.join(
      ...[
        this.getBucketPath(bucket),
        key,
        format(S3RVER_SUFFIX, `${resourceType}.xml`)
      ].filter(Boolean)
    );
  ...
  async storeSubresource(bucket, key, resource) {
    const resourcePath = path.join(
      ...[
        this.getBucketPath(bucket),
        key,
        format(S3RVER_SUFFIX, `${resource.type}.xml`)
      ].filter(Boolean)
    );
