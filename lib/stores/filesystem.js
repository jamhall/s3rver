...
class FilesystemStore {
  ...
  async putObjectTagging(bucket, key, content) {
    const taggingPath = this.getResourcePath(bucket, key, "tagging.json");
    const parsedTagging = await parseXmlBody(content);

    const json = {
      TagSet: parsedTagging.Tagging.TagSet
    };

    await fs.writeFile(taggingPath, JSON.stringify(json, null, 2));
  }

  async getObjectTagging(bucket, key) {
    const taggingPath = this.getResourcePath(bucket, key, "tagging.json");
    const storedTagging = await fs
      .readFile(taggingPath)
      .then(JSON.parse)
      .catch(err => {
        if (err.code === "ENOENT") return undefined;
