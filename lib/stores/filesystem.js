...
const OBJECT_FILE = "._S3rverObject";
...
class FilesystemStore {
  ...
  async retrieveMetadata(bucket, key) {
    const contentPath = path.join(this.getBucketPath(bucket), key, OBJECT_FILE);
  ...
  async getBucket(bucket) {
    ...
    try {
      ...
      return new Bucket(bucket, stat.birthtime);
    } catch (err) {
  ...
  async existsObject(bucket, key) {
    const objFile = path.join(this.getBucketPath(bucket), key, OBJECT_FILE);
    try {
  ...
  async getObject(bucket, key, options) {
    ...
    try {
      ...
      const content = await new Promise((resolve, reject) => {
        const stream = fs
          .createReadStream(path.join(dirName, OBJECT_FILE), range)
          .on("error", reject)
  ...
  async putObject(object) {
    ...
    const [size, md5] = await new Promise((resolve, reject) => {
      const writeStream = fs.createWriteStream(path.join(dirName, OBJECT_FILE));
      const md5Context = crypto.createHash("md5");
      let totalLength = 0;
  ...
  async copyObject(
    ...
    const srcContentPath = path.join(srcKeyPath, OBJECT_FILE);
    const destContentPath = path.join(destKeyPath, OBJECT_FILE);
  ...
  async deleteObject(bucket, key) {
    ...
    await Promise.all(
      [
        path.join(keyPath, OBJECT_FILE),
        path.join(keyPath, `${OBJECT_FILE}.md5`),
        path.join(keyPath, `${OBJECT_FILE}.json`)
      ].map(filePath =>
  ...
  async putObjectMultipart(bucket, key, uploadId, parts, metadata) {
    ...
    const partStreams = partPaths.map(partPath => {
      const stream = fs.createReadStream(path.join(partPath, OBJECT_FILE));
