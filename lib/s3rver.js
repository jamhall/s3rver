...
class S3rver extends Koa {
  constructor(options) {
    ...
    const {
      ...
      resetOnClose,
      configureBuckets,
      forceVirtualHostnames,
      enableSubpathMounting,
      ...serverOptions
    ...
    try {
      ...
      this.use(vhostMiddleware(forceVirtualHostnames));
      this.use(corsMiddleware());
      this.use(websiteMiddleware());
      this.use(authenticationMiddleware(enableSubpathMounting));
  ...
  /**
   * this initialises buckets, and returns a callback that can be used as middleware.
   */
  callback() {
...
S3rver.defaultOptions = {
  ...
  key: undefined,
  cert: undefined,
  forceVirtualHostnames: false,
  enableSubpathMounting: false,
  silent: false,
