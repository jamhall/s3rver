...
async function xmlBodyParser(ctx) {
  ...
  ctx.req.body = xmlParser.parse(xmlString);
}
...
exports.deleteMultipleObjects = async function deleteMultipleObjects(ctx) {
  ...
  if (!ctx.req.body.Delete || !ctx.req.body.Delete.Object) {
    ctx.throw(
      400,
      "The XML you provided was not well-formed or did not validate against our published schema",
      { code: "MalformedXML" }
  ...
  const keys = [].concat(ctx.req.body.Delete.Object).map(o => o.Key);
  await Promise.all(
    keys.map(async key => {
...
exports.getObject = async function getObject(ctx) {
  ...
  let object = await ctx.store.getObject(ctx.params.bucket, key, options);
  if (!object) {
    ctx.throw(404, "The specified key does not exist.", {
      code: "NoSuchKey",
      detail: { Key: key }
    });
  }
  ...
  if (object.range && !object.content) {
    ctx.throw(416, "The requested range is not satisfiable", {
      code: "InvalidRange",
      detail: {
...
exports.putObjectCopy = async function putObjectCopy(ctx) {
  ...
  try {
    ...
    if (!bucket) {
      ...
      ctx.throw(404, "The specified bucket does not exist", {
        code: "NoSuchBucket",
        resource: srcBucket
      });
    }
    ...
    if (srcBucket === destBucket && srcKey === destKey && !replaceMetadata) {
      ctx.throw(
        ...
        "This copy request is illegal because it is trying to copy an object to itself " +
          "without changing the object's metadata, storage class, website redirect location or " +
...
exports.completeMultipartUpload = async function completeMultipartUpload(ctx) {
  await xmlBodyParser(ctx);
  const parts = []
    .concat(ctx.req.body.CompleteMultipartUpload.Part)
    .map(part => ({
...
exports.uploadPart = async function uploadPart(ctx) {
  const partKey = ctx.query.uploadId + "_" + ctx.query.partNumber;
  const object = new S3Object(ctx.params.bucket, partKey, ctx.req, ctx.headers);
  try {
    const { md5 } = await ctx.store.putObject(object);
    ctx.logger.info(
      'Stored object "%s" in bucket "%s" successfully',
      partKey,
      ctx.params.bucket
