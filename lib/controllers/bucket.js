...
exports.deleteBucket = async function deleteBucket(ctx) {
  try {
    ...
    ctx.throw(409, "The bucket your tried to delete is not empty", {
      code: "BucketNotEmpty",
      detail: { BucketName: ctx.params.bucket }
    });
  }
};
...
exports.putBucket = async function putBucket(ctx) {
  ...
  if (bucketName.length < 3 || 63 < bucketName.length) {
    ...
    ctx.throw(400, "The bucket name must be between 3 and 63 characters.", {
      code: "InvalidBucketName"
    });
  }
  ...
  if (bucket) {
    ...
    ctx.throw(409, "The requested bucket already exists", {
