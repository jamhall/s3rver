  ...
  return {
    ...
    putBucket: function (req, res) {
      ...
      if ((/^[a-z0-9]+(-[a-z0-9]+)*$/.test(bucketName) === false)) {
        var template = templateBuilder.buildError('BucketNameInvalid',
            'Bucket names can contain lowercase letters, numbers, and hyphens. ' +
            'Each label must start and end with a lowercase letter or a number.');
        return buildXmlResponse(res, 400, template);
      }
      if (bucketName.length < 3 || bucketName.length > 63) {
        var template = templateBuilder.buildError('BucketNameLength',
    ...
    putObject: function (req, res) {
      fileStore.storeKey(req.bucket, req, function (err, key) {
        if (err) {
